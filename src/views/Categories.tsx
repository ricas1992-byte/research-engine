...
type AddingSubCatFor = string | null;
type AddingSubQFor = string | null;
...
function SubQRow({
  sq, idx, isFirst, isLast, readOnly,
      ...
      {isEditing ? (
        <div className="flex-1 space-y-2">
          <div className="flex gap-2">
            <input autoFocus value={editingSubQ!.number}
          ...
          <textarea value={editingSubQ!.text}
          ...
          <input value={editingSubQ!.description}
          ...
          <div className="flex gap-2">
            <button type="button" onClick={onSaveEdit}
              className="px-3 py-1 bg-indigo-600 text-white rounded-lg text-xs hover:bg-indigo-700">שמור</button>
            <button type="button" onClick={onCancelEdit}
              className="px-3 py-1 text-slate-600 rounded-lg text-xs hover:bg-slate-100">ביטול</button>
          </div>
        </div>
      ) : (
        <>
...
function InlineEdit({ value, onSave, className = '', placeholder = '', multiline = false, readOnly = false }: InlineEditProps) {
  ...
  if (readOnly || !editing) {
    return (
      <span
        className={`${!readOnly ? 'cursor-pointer hover:bg-amber-50' : ''} rounded px-1 -mx-1 transition-colors ${className}`}
        onClick={() => { if (!readOnly) { setDraft(value); setEditing(true); } }}
        title={readOnly ? undefined : 'לחץ לעריכה'}
      >
      ...
      <div className="flex items-start gap-2 bg-amber-50 px-4 py-3 border-b border-amber-100">
        {!readOnly && (
          <div className="flex flex-col gap-0.5 flex-shrink-0 mt-0.5">
            <button type="button" onClick={() => onMoveSubCatUp(sc.id)} disabled={isFirst}
              ...
              title="הזז למעלה">▲</button>
            <button type="button" onClick={() => onMoveSubCatDown(sc.id)} disabled={isLast}
              className="w-5 h-5 flex items-center justify-center text-amber-400 hover:text-amber-700 hover:bg-amber-100 rounded disabled:opacity-25 disabled:cursor-not-allowed text-xs leading-none"
              title="הזז למטה">▼</button>
          </div>
        )}
        <div className="flex-1 min-w-0">
          <InlineEdit value={sc.name} onSave={(v) => v && onUpdateSubCat(sc.id, { name: v })}
            className="font-semibold text-amber-900 text-sm" placeholder="שם תת-קטגוריה" readOnly={readOnly} />
          {sc.description !== undefined && (
            <div className="mt-0.5">
              <InlineEdit value={sc.description} onSave={(v) => onUpdateSubCat(sc.id, { description: v || undefined })}
                className="text-xs text-amber-700" placeholder="תיאור (לחץ להוספה)" multiline readOnly={readOnly} />
            </div>
          )}
        ...
        {!readOnly && (
          <button type="button"
            onClick={() => { if (confirm(`למחוק תת-קטגוריה "${sc.name}"?\nשאלות המשנה שלה יישארו ויועברו ישירות לקטגוריה.`)) onDeleteSubCat(sc.id); }}
            className="p-1.5 text-amber-400 hover:text-red-600 hover:bg-red-50 rounded-lg flex-shrink-0"
            title="מחק תת-קטגוריה">🗑</button>
        )}
      </div>
      <div className="p-3 space-y-2 bg-white">
        {sqs.map((sq, idx) => (
          <SubQRow key={sq.id} sq={sq} idx={idx}
            isFirst={idx === 0} isLast={idx === sqs.length - 1}
            readOnly={readOnly}
            ...
            editingSubQ={editingSubQ} onSaveEdit={onSaveSubQ}
            onCancelEdit={() => setEditingSubQ(null)} setEditingSubQ={setEditingSubQ} />
        ))}
        {sqs.length === 0 && (
...
function AddSubQForm({ subCategories, onAdd, onCancel }: AddSubQFormProps) {
  ...
  return (
    <div className="space-y-2 pt-1 border border-indigo-100 rounded-xl p-3 bg-indigo-50/40">
      <div className="flex gap-2">
        <input dir="ltr" value={number} onChange={(e) => setNumber(e.target.value)}
          placeholder="מספר (למשל 0.1)"
          className="w-28 border border-slate-300 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
        {subCategories.length > 0 && (
          <select value={subCatId ?? ''} onChange={(e) => setSubCatId(e.target.value || null)}
            title="בחר תת-קטגוריה"
            aria-label="בחר תת-קטגוריה"
      ...
      <div className="flex gap-2">
        <button type="button" onClick={handle}
          className="px-3 py-1.5 bg-indigo-600 text-white rounded-lg text-xs hover:bg-indigo-700">הוסף שאלה</button>
        <button type="button" onClick={onCancel}
          className="px-3 py-1.5 text-slate-600 rounded-lg text-xs hover:bg-slate-100">ביטול</button>
      </div>
    </div>
  );
}
...
function AddSubCatForm({ onAdd, onCancel }: AddSubCatFormProps) {
  ...
  return (
    <div className="space-y-2 border border-amber-200 rounded-xl p-3 bg-amber-50/40">
      ...
      <div className="flex gap-2">
        <button type="button" onClick={handle}
          className="px-3 py-1.5 bg-amber-500 text-white rounded-lg text-xs hover:bg-amber-600">הוסף תת-קטגוריה</button>
        <button type="button" onClick={onCancel}
          className="px-3 py-1.5 text-slate-600 rounded-lg text-xs hover:bg-slate-100">ביטול</button>
      </div>
    </div>
...
export function Categories() {
  ...
  const readOnly = tab === 'archive';

  // Filter categories by project
  const archivedProjectIds = new Set(getArchivedProjects().map((p) => p.id));

  const visibleCategories = categories.filter((cat) => {
    const pid = getProjectIdForCategory(cat.id);
    if (tab === 'active') return pid === activeProjectId;
    return archivedProjectIds.has(pid);
  });
  ...
  const handleAddCategory = async () => {
    ...
    assignCategoryToProject(saved.id, activeProjectId);
    ...
    <div className="p-8 max-w-3xl mx-auto space-y-6" dir="rtl">
      ...
      {tab === 'archive' && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 text-sm text-amber-700">
          <span className="font-semibold">מצב ארכיון — קריאה בלבד.</span> קטגוריות אלו שייכות לפרויקטים מאורכבים.
        </div>
      )}

      {/* Add category form */}
      {showAddCategory && tab === 'active' && (
        <div className="bg-white border border-indigo-200 rounded-xl p-5 shadow-sm space-y-3">
          ...
          <div className="flex gap-2">
            <button type="button" onClick={handleAddCategory}
              className="px-4 py-1.5 bg-indigo-600 text-white rounded-lg text-sm hover:bg-indigo-700">שמור</button>
            <button type="button" onClick={() => setShowAddCategory(false)}
              className="px-4 py-1.5 text-slate-600 rounded-lg text-sm hover:bg-slate-100">ביטול</button>
          </div>
        </div>
      )}

      {visibleCategories.length === 0 && !showAddCategory && (
        <p className="text-slate-400 text-sm text-center py-8">
          {tab === 'active'
            ? 'אין קטגוריות בפרויקט הפעיל. לחץ "קטגוריה חדשה" להתחיל.'
            : 'אין קטגוריות מאורכבות.'}
        </p>
      )}
      ...
      {visibleCategories.map((cat) => {
        ...
        return (
          <div key={cat.id} className={`bg-white border rounded-xl shadow-sm overflow-hidden ${readOnly ? 'border-amber-200 opacity-90' : 'border-slate-200'}`}>
            {/* Category header */}
            <div className="flex items-start gap-3 p-5 border-b border-slate-100">
              <div className="w-3 h-3 rounded-full mt-1.5 flex-shrink-0" style={{ backgroundColor: cat.color }} />
              {isEditingCat ? (
                <div className="flex-1 space-y-2">
                  <input autoFocus placeholder="שם הקטגוריה" value={editingCat.name}
                    onChange={(e) => setEditingCat({ ...editingCat, name: e.target.value })}
                    className="w-full border border-slate-300 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                  <textarea value={editingCat.description}
                    onChange={(e) => setEditingCat({ ...editingCat, description: e.target.value })}
                    placeholder="תיאור" rows={2}
                    className="w-full border border-slate-300 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none" />
                  <ColorPicker value={editingCat.color}
            ...
            <div className="p-4 space-y-2">
              {catSubCats.map((sc, scIdx) => (
                <SubCatSection key={sc.id} sc={sc} sqs={getSubQuestionsBySubCategory(sc.id)}
                  readOnly={readOnly}
                  editingSubQ={editingSubQ} setEditingSubQ={setEditingSubQ}
                  ...
                  isFirst={scIdx === 0} isLast={scIdx === catSubCats.length - 1}
              ...
              {directSQs.length > 0 && (
                <div className="space-y-2">
                  ...
                  {directSQs.map((sq, idx) => (
                    <SubQRow key={sq.id} sq={sq} idx={idx}
                      isFirst={idx === 0} isLast={idx === directSQs.length - 1}
                      readOnly={readOnly}
                      onMoveUp={() => moveSubQuestionUp(sq.id)}
                      onMoveDown={() => moveSubQuestionDown(sq.id)}
                      onEdit={() => setEditingSubQ({ id: sq.id, text: sq.text, description: sq.description ?? '', number: sq.number ?? '' })}
                      onDelete={() => { if (confirm('למחוק שאלת משנה זו וכל החקירות שלה?')) deleteSubQuestion(sq.id); }}
                      editingSubQ={editingSubQ} onSaveEdit={handleUpdateSubQ}
                      onCancelEdit={() => setEditingSubQ(null)} setEditingSubQ={setEditingSubQ} />
                  ))}
                </div>
              )}
              ...
              {!readOnly && (
                <>
                  {addingSubCatFor === cat.id ? (
                    <AddSubCatForm onAdd={(name, desc) => handleAddSubCat(cat.id, name, desc)}
                      onCancel={() => setAddingSubCatFor(null)} />
                  ) : null}

                  {addingSubQFor === cat.id ? (
                    <AddSubQForm subCategories={catSubCats}
                      onAdd={(text, desc, number, subCatId) => handleAddSubQ(cat.id, text, desc, number, subCatId)}
